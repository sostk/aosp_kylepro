...
OMXPluginBase *createOMXPlugin() {
    return new BrcmOMXPlugin;
}

BrcmOMXPlugin::BrcmOMXPlugin()
    : mLibHandle(dlopen("libBrcmOMX_Core.so", RTLD_NOW)),
      mInit(NULL),
      mDeinit(NULL),
...
OMX_ERRORTYPE BrcmOMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE BrcmOMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE BrcmOMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE BrcmOMXPlugin::getRolesOfComponent(
        const char *name,
